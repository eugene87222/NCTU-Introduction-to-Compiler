...
__BOOLEAN verifyScalarExpr(struct expr_sem *expr, const char *str) {
    if(expr->pType->dim > 0) {
...
__BOOLEAN verifyReturnStatement(struct expr_sem *expr, struct PType *funcReturn) {
    if(funcReturn == 0) {
...
void verifyEntryFunc() {
    if(isEntryFunc) VarIdx = 1;
    else VarIdx = 0;
}